...
index = tf.where(tf.greater(x, 30))
index_1d = tf.reshape(index, [-1])
ele = tf.gather(x, index_1d)
print(sess.run([index_1d,ele]))
...
random = tf.random_uniform([10,10],0,10)
...
f1 = lambda : tf.reduce_sum(tf.square(x - y), 0)
f2 = lambda : tf.reduce_sum(tf.abs(x - y), 0)

out = tf.cond(cond, f1, f2)
print(sess.run(out))